...
std::unique_ptr<int, decltype(del1)> u1 = std::move(u2);  // ✅ Same type: move-construction compiles
...
**⚠️ Lambda-deleter caveat:** Move-*construction* of two same-type, lambda-deleter unique_ptrs (as shown above) compiles fine. However, move-*assignment* between two already-constructed unique_ptrs with a lambda deleter does **not** compile:
```cpp
std::unique_ptr<int, decltype(del1)> a(new int(1), del1);
std::unique_ptr<int, decltype(del1)> b(new int(2), del1);
a = std::move(b);  // ❌ Error: use of deleted function operator=(unique_ptr&&)
```
This is because closure types generated by lambda expressions have an implicitly **deleted** copy/move assignment operator (their copy/move constructors are still implicitly generated, but assignment is not). Since unique_ptr's defaulted move-assignment operator needs to move-assign the deleter member, it becomes deleted too whenever the deleter is a lambda type. Using a function pointer or a hand-written functor (with an explicit or defaulted `operator=`) as the deleter instead of a lambda avoids this and allows move-assignment to work normally.
