...
public:
    ...
    void unsubscribe(EventHandler h) {
        handlers.erase(/*..*/);  // Modifies vector!
    }
};
```
...
**Bug:** Iterator invalidation - modifying vector while iterating crashes!

**Fix:** Copy handlers before iterating:
```cpp
void publish(const Event& e) {
    auto copy = handlers;  // Snapshot
    for (auto& h : copy) h(e);  // Safe
}
```
...
#### Additional Practice Tasks 2-20:
- Fix event loop (infinite recursion)
- Implement priority queue correctly
- Add thread safety
- Handle exceptions in handlers
- Implement event coalescing
- Design event sourcing system
- Create RAII subscription handle
- And 13 more challenges...

*(Detailed solutions in complete file)*
