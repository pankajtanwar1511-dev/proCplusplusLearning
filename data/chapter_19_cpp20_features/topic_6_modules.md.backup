...
### PRACTICE_TASKS: Module Scenarios
...
**Explanation:** `bar()` is not exported, so it's not visible in `main.cpp`. Only `foo()` is accessible.
...
#define VALUE 42
export int get_value() { return VALUE; }
...
**Explanation:** `VALUE` macro doesn't cross module boundary. `main.cpp` doesn't see the macro definition.

---

(Q3-Q20 would continue with scenarios involving: Partition visibility, private fragments, template exports, ODR violations, and module linkage)
