...
**Explanation:**
- `42` is `int` → integral
- `3.14` is `double` → floating_point
- `true` is `bool` → integral type in C++!

**Key Concept:** #concepts #overload_resolution #bool_is_integral
...
**Explanation:**
- `42` is `int` (signed integral) → satisfies both A and B, but B subsumes A → "B"
- `42u` is `unsigned int` → satisfies A but not B → "A"

**Key Concept:** #subsumption #overload_resolution #signed_unsigned

</details>

---

**[Q3-Q50 continue with progressively complex scenarios testing concept composition, subsumption, abbreviated templates, requires expressions, standard concepts, and edge cases...]**
